...
public class Project098 {
	final static String[][] anagramWords = {
			{"ACT", "CAT"},
			{"ARISE", "RAISE"},
			{"BOARD", "BROAD"},
			{"CARE", "RACE"},
...
class Word {
	...
	public static void findAnagramWords() {
		...
		for (int i = 0; i < words.length - 1; i++) {
			for (int j = i + 1; j < words.length; j++) {
				if (words[i].getWord().equals(words[j].getWord())) {
					System.out.println("{\"" + str[words[i].getId()] + "\",\"" + str[words[j].getId()] + "\"},");
				}
			}
		}
	}
